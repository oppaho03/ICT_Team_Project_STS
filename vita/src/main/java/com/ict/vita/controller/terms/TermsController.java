...
import com.ict.vita.service.terms.FindTermDto;
...
public class TermsController {
	...
	public ResponseEntity<?> getAllByCategory( @Parameter(description = "카테고리(Taxonomy)") @PathVariable String taxonomy ) { return ResponseEntity.status(HttpStatus.OK).body(ResultUtil.success( termsService.findAllByTaxonomy( taxonomy ) )); }

	/**
	 * ID 검색 
	 * @return TermDto 또는 Null 반환
	 */	
	@Operation( summary = "ID 검색 ", description = "ID 검색" )
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "SUCCESS", content = @Content(schema = @Schema(implementation = TermDto.class), examples = @ExampleObject(value = "{\"success\":1,\"response\":{\"data\":{\"id\":1288,\"term_id\":1260,\"name\":\"전이성신장암\",\"slug\":\"%EC%A0%84%EC%9D%B4%EC%84%B1%20%EC%8B%A0%EC%9E%A5%EC%95%94\",\"group_number\":0,\"category\":\"disease\",\"description\":null,\"count\":0,\"parent\":1179}}}"))),
	...
	@GetMapping("/s")
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "SUCCESS", content = @Content(schema = @Schema(implementation = TermDto.class), examples = @ExampleObject(value = "{\"success\":1,\"response\":{\"data\":{\"id\":1466,\"term_id\":1438,\"name\":\"정신의학과\",\"slug\":\"%EC%A0%95%EC%8B%A0%EC%9D%98%ED%95%99%EA%B3%BC\",\"group_number\":0,\"category\":\"department\",\"description\":null,\"count\":0,\"parent\":0}}}"))),
		@ApiResponse(responseCode = "200", description = "SUCCESS", content = @Content(examples = @ExampleObject(value = "{\"success\":1,\"response\":{\"data\":[]}}"))),
		@ApiResponse(responseCode = "200", description = "SUCCESS", content = @Content(schema = @Schema(implementation = TermDto.class), examples = @ExampleObject(value = "{\"success\":1,\"response\":{\"data\":[{\"id\":1404,\"term_id\":1376,\"name\":\"객혈\",\"slug\":\"%EA%B0%9D%ED%98%88\",\"group_number\":0,\"category\":\"disease\",\"description\":null,\"count\":0,\"parent\":1375},{\"id\":1405,\"term_id\":1377,\"name\":\"공기가슴증(기흉)\",\"slug\":\"pneumothorax\",\"group_number\":0,\"category\":\"disease\",\"description\":null,\"count\":0,\"parent\":1375},{\"id\":1406,\"term_id\":1378,\"name\":\"규폐증\",\"slug\":\"%EA%B7%9C%ED%8F%90%EC%A6%9D\",\"group_number\":0,\"category\":\"disease\",\"description\":null,\"count\":0,\"parent\":1375},{\"id\":1407,\"term_id\":1379,\"name\":\"급성기관지염\",\"slug\":\"acute_bronchitis\",\"group_number\":0,\"category\":\"disease\",\"description\":null,\"count\":0,\"parent\":1375}]}}"))),
	...
	public ResponseEntity<?> getBySlug( 
		...
		if ( ! Commons.isNull(slug) && ! Commons.isNull(category) ) {
			...
			try {
				...
				return ResponseEntity.status(HttpStatus.OK).body(ResultUtil.success( termsService.findBySlugByCategory( slug, category ) )); 
			}
			catch ( Exception e ) {
	...
	public ResponseEntity<?> add( @Parameter( description = "Term 데이터") @RequestBody @Valid EmptyTermDto dto, BindingResult bindingResult ) {
		...
		try {
			...
			TermDto result = termsService.save(dto);
			if ( result == null ) {
				// - 슬러그 중복 (등록 중단)
				throw new Exception(
					String.format(
