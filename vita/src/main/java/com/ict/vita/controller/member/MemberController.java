...
public class MemberController {
	...
	@ApiResponses({
		...
		@ApiResponse( 
			...
			content = @Content(					
				examples = @ExampleObject(
					value = "{ \"success\": 0, \"response\": { \"message\": \"Invalid Values\"  } }" 
				)
			) 
		),
		@ApiResponse( 
			responseCode = "409", 
			description = "FAIL-전화번호 또는 이메일 중복", 
			content = @Content(					
				examples = @ExampleObject(
						value = "{ \"success\": 0, \"response\": { \"message\": \"이미 사용 중인 이메일/전화번호입니다.\"  } }" )
			) 
		)
	})
	@PostMapping("/member")
