...
import com.ict.vita.service.chatquestion.ChatQuestionWithSessionDto;
...
public class ChatQuestionController {
	...
	public ResponseEntity<?> createQuestionWithSession(@RequestBody ChatQuestionWithSessionDto qwsDto,@RequestHeader(name = "Authorization") String token){
		//회원의 토큰값 조회
		...
		//질문의 키워드로 답변 검색
		List<ChatAnswerDto> answerList = chatAnswerService.findAnswerByKeywords(qwsDto.getKeywords().stream().collect(Collectors.joining(" OR ")));
		//<답변 검색 결과가 없는 경우>
